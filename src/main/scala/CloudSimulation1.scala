...
object CloudSimulation1 {
  ...
  @SuppressWarnings(Array("unused")) def main(args: Array[String]): Unit = {
    logger.debug("Starting CloudSimulation1...")
    try { // First step: Initialize the CloudSim package. It should be called
      ...
      val datacenter1 = createDatacenter("datacenter1")
      val datacenter2 = createDatacenter("datacenter2")
      val datacenter3 = createDatacenter("datacenter3")
      val datacenter4 = createDatacenter("datacenter4")
      ...
      logger.debug("CloudSimulation1 finished!")
    } catch {
      case e: Exception =>
        e.printStackTrace()
        logger.debug("Unwanted errors happen")
    }
  }
...
// submit vm list to the broker
  ...
  private def printCloudletList(list: util.List[_<:Cloudlet]): Unit = {
    ...
    range.foreach(i => {
      ...
      val ourCost = cloudlet.getProcessingCost + cloudlet.
